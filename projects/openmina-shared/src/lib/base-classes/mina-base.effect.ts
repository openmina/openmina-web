...
export abstract class MinaBaseEffect<A extends FeatureAction<any>, State> {
  ...
  protected readonly latestStateSlice = <Slice extends object, Action extends A>(path: string): OperatorFunction<Action, Slice> => selectLatestStateSlice<Slice, State, Action>(this.store, this.selector, path);
