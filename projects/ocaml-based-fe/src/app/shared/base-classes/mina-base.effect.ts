...
import { selectActionAndState, selectLatestStateSlice } from '@ocfe-shared/constants/store-functions';
...
export abstract class MinaBaseEffect<A extends FeatureAction<any>> {
  ...
  protected readonly latestStateSlice = <Slice extends object, Action extends A>(path: string): OperatorFunction<Action, Slice> => selectLatestStateSlice<Slice, MinaState, Action>(this.store, this.selector, path);
