...
export function reducer(state: SystemResourcesState = initialState, action: SystemResourcesActions): SystemResourcesState {
  switch (action.type) {
    case SYSTEM_RESOURCES_GET_RESOURCES_SUCCESS: {
      ...
      if (state.activeTime) {
        pointFromRoute = getClosestPoint(any(action.payload)[state.activeResource], state.activeTime);
      }
      const activePoint = pointFromRoute
        ? {
          point: pointFromRoute,
          title: any(state.chartData)[state.activeResource + 'Title'],
          colors: any(state.colorMapping)[state.activeResource],
          um: any(state.chartData)[state.activeResource + 'Um'],
        }
        : {
