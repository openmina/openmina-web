...
export class SystemResourcesService {
  ...
  private static mapSystemResourcesResponse(response: any[]): SystemResourcesChartData {
    ...
    const cpuNames: { [parent: string]: any } = {};
    const memoryNames: { [parent: string]: any } = {};
    const ioNames: { [parent: string]: any } = {};
    const networkNames: { [parent: string]: any } = {};
      ...
      resources.cpu.push({
        ...
        pathPoints: Object.keys(cpuNames).reduce((final: any, key: string) => ({
          ...
          [key]: {
            ...
            taskThreads: Object.entries<number>(item.cpu.subprocesses.subprocesses[key]?.task_threads || {}).map(e => ({
              name: e[0],
              value: e[1]
            })),
          },
  ...
  private static getMax(key: keyof SystemResourcesChartData, resources: SystemResourcesChartData): number {
    return niceYScale(0, Math.max(
      ...(resources[key] as []).reduce((acc: number[], curr: SystemResourcesPoint) => [...acc, ...Object.values(curr.pathPoints).map(v => v.value)], []),
    ), 3)[1];
  }
}
