...
import { any, ONE_THOUSAND, SortDirection } from '@openmina/shared';
            ...
            .then((state: SnarkWorkersTracesState) => {
              if (state && jobs.length) {
                ...
                cy.get('mina-snark-workers-side-panel > div .times > div:nth-child(1) > span:nth-child(2)')
                  ...
                  .should('contain', any(state.workers)[jobs[0].worker])
                  .log('')
                  .url()
          ...
          cy.get('.mina-table')
            ...
            .then((spans: any) => {
              Array.from(spans).forEach((span: any, i: number) => {
                expect(span.innerText).to.equal(truncateMid(any(state.workers)[jobs[i].worker], 0, 12));
              });
            });
        }
      });
